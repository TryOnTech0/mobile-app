...
{
    // Connect NetworkManager signals to QMLManager slots
...
// Add this slot to handle received garments
void QMLManager::handleGarmentsReceived(const QJsonArray& garments) {
    m_garments.clear();
    for (const auto& garment : garments) {
        QVariantMap entry;
        entry["id"] = garment.toObject().value("_id").toString();
        entry["name"] = garment.toObject().value("name").toString();
        entry["previewUrl"] = garment.toObject().value("previewUrl").toString();
